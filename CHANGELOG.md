...
- **Triggers UI in Watches page** — Full IFTTT-style trigger management integrated into Watches page. Two-tab layout (Watches + Triggers) with dashboard stats (active watches, total alerts, active triggers, fires count). Create trigger dialog with event presets (Workflow fails, Session completes, Agent task done, Watch alerts, Heartbeat, Document ingested, Custom), action types (Run prompt, Run workflow, Notify), cooldown selector. Real-time polling (3s triggers, 5s watches). Enable/disable toggle per trigger.
- **Triggers API client** — Frontend `api.triggers` namespace: list, create, delete, toggle, testEvent
